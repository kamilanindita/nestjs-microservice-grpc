...
import { CustomerResponse, CustomerResponses } from './dto/grpc-customer.dto';
...
export class CustomerService {
    ...
    findAll(): Promise<CustomerResponses> {
        return new Promise(resolve => {
            resolve({'data': this.customers});
        });
    }

    findOne(id: number): Promise<CustomerResponse|string>{
        return new Promise(resolve => {
    ...
    update(id: number, updateCustomerDTO: UpdateCustomerDTO): Promise<CustomerResponse|string> {
        return new Promise(resolve => {
    ...
    delete(id: number): Promise<CustomerResponse|string> {
        return new Promise(resolve => {
            const customerIndex = this.customers.findIndex(customer => customer.id === id);
            if (customerIndex === -1) {
                resolve('Customer does not exist!');

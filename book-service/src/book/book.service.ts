...
import { BookResponse, BookResponses } from './dto/grpc-book.dto';
...
export class BookService {
    ...
    findAll(): Promise<BookResponses> {
        return new Promise(resolve => {
            resolve({'data': this.books});
        });
    }

    findOne(id: number): Promise<BookResponse|string>{
        return new Promise(resolve => {
    ...
    update(id: number, updateBookDTO: UpdateBookDTO): Promise<BookResponse|string> {
        return new Promise(resolve => {
    ...
    delete(id: number): Promise<BookResponse|string> {
        return new Promise(resolve => {
            const bookIndex = this.books.findIndex(book => book.id === id);
            if (bookIndex === -1) {
                resolve('Book does not exist!');
